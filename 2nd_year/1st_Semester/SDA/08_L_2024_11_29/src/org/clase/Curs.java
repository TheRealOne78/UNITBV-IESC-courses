    ...
    public void updateStudent(Student student_old, Student student_nou) {
        for(Student s : this.studenti) {
            if(s.equals(student_old))
                s = student_nou; // This won't modify the actual student object in the set.
        }
    }
    ...
    public void removeStudent(Student student) {
        for (var tmp : studenti) {
            if (tmp.equals(student))
                studenti.remove(tmp); // Removing from set during iteration is dangerous and can throw ConcurrentModificationException
        }
    }
